...
const onSubmit = (values, { setSubmitting, setErrors, setFieldError }) => {
  if (!isNullOrUndefined(values.datafile) && (values.datafile.type === 'text/csv' || values.datafile.type === 'application/vnd.ms-excel') || values.datafile.type === 'text/plain' ) {
    ...
    axios.post('/api/clientgroups/import', fd, {
      headers: { ...authHeader() }
    })
      .then(function (response) {
        ...
        if (values.importResults.length === 1) {
          ...
          values.updateNoticeMessage = 'Client groups imported';
      ...
      .catch(function (error) {
        ...
        if (error.response.status === 500) {
          ...
          values.updateNoticeVisible = true;
          values.updateNoticeMessage = 'Problem processing client group import';
          values.updateNoticeStyle = 'danger';
          setSubmitting(false);
        }
      });
...
class ClientGroupsImport extends Component {
  constructor(props) {
    super(props);
    this.state = {
      datafile: null,
            ...
            render={
              ({
                ...
                <Form onSubmit={handleSubmit} noValidate name='simpleForm'>
                  <Card>
                    <CardHeader><i className="fa fa-user" /><strong>Client Group Import</strong>
                    </CardHeader>
                    <CardBody>
                      <FormGroup row>
                        <Col md="2">
                        ...
                        <Col xs="12" md="10">
                          <Input type="file"
                              ...
                              name="datafile"
                              id="datafile"
                              placeholder="Client Group Data File"
                          ...
                          <FormFeedback>{errors.datafile}</FormFeedback>
                            <FormText>The import file must be in a TAB delimited format with these two fields [Your client group id] [Client group name]</FormText>
                        </Col>
                      </FormGroup>
                    </CardBody>
                    <CardFooter>
...
export default ClientGroupsImport;
