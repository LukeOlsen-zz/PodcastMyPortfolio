...
const onSubmit = (values, { setSubmitting, setErrors, setFieldError }) => {
  if (isNullOrUndefined(values.audiofile) || values.audiofile.type === 'audio/mpeg') {
    ...
    if (!isNullOrUndefined(values.audiofile)) {
      fd.append("file", values.audiofile, 'clientgrouppodcastsegment.mp3');
    }
    ...
    fd.append("clientgroupid", values.clientgroup.id);

    if (values.id === 0) {
      axios.post('/api/clientgrouppodcastsegments/create', fd, {
        headers: { ...authHeader() }
      })
        .then(function (response) {
          setTimeout(() => {
            ...
            values.updateNoticeMessage = 'Group podcast segment created';
        ...
        .catch(function (error) {
          ...
          if (error.response.status === 500) {
            ...
            values.updateNoticeVisible = true;
            values.updateNoticeMessage = 'Group podcast segment not created';
      ...
      axios.put('/api/clientgrouppodcastsegments/update', fd, {
        headers: { ...authHeader() }
      })
        .then(function (response) {
          setTimeout(() => {
            ...
            values.updateNoticeVisible = true;
            values.updateNoticeMessage = 'Group podcast segment updated';
            values.updateNoticeStyle = 'success';
            setSubmitting(false);
          }, 1000);
        })
        .catch(function (error) {
          ...
          if (error.response.status === 500) {
            ...
            values.updateNoticeVisible = true;
            values.updateNoticeMessage = 'Group podcast segment not updated';
            values.updateNoticeStyle = 'danger';
            setSubmitting(false);
          }
        });
...
class ClientGroupPodcastSegment extends Component {
  constructor(props) {
    super(props);
    this.state = {
      id: 0,
  ...
  componentDidMount() {
    ...
    if (params.id !== '0') {
      axios.get(`/api/clientgrouppodcastsegments/${params.id}`, { headers: { ...authHeader(), 'Content-Type': 'application/json' } })
        .then(response => {
          if (response.status === 200) {
            ...
            var clientGroupId = response.data.clientGroupId;

            document.getElementById("deleteLink").style.visibility = "visible";

            // Lets set the current value of the client group drop down
            axios.get(`/api/clientgroups/clientgroup/${clientGroupId}`, { headers: { ...authHeader(), 'Content-Type': 'application/json' } })
              .then(response => {
                if (response.status === 200) {
                  this.setState({ selectClientGroup: response.data });
                }
              })
              .catch(error => {
  ...
  loadOptions(inputValue, callback) {
    axios.get(`/api/clientgroups?name=${inputValue}&size=7`, { headers: { ...authHeader(), 'Content-Type': 'application/json' } })
      .then(response => {
        if (response.status === 200) {
  ...
  onDelete() {
    axios.delete(`/api/clientgrouppodcastsegments/${this.state.id}`, { headers: { ...authHeader(), 'Content-Type': 'application/json' } })
      .then(response => {
          this.props.history.push('/podcasts/clientgrouppodcastsegments');
      })
      .catch(function (error) {
        console.log(error);
        if (error.response.status === 409) {
          ...
          alert('There was a problem deleting this group podcast segement ' + error.response);
      });
  }
      ...
      <div className="animated fadein">
        <Formik
            ...
            initialValues={{
              ...
              endson: this.state.endson, audiofile: null, clientgroup: this.state.selectClientGroup, updateNoticeVisible: false, updateNoticeMessage: '', updateNoticeStyle: 'success'
            }}
                        ...
                        <Col md="2">
                          ...
                          <Col xs="12" md="10">
                            <Async className="is-valid"
                                ...
                                isSearchable
                                valid={!errors.clientgroup}
                                invalid={touched.clientgroup && !!errors.clientgroup}
                                onChange={e => { this.handleGroupChange(e, setFieldValue); values.updateNoticeVisible = false; }}
...
export default withRouter(ClientGroupPodcastSegment);
