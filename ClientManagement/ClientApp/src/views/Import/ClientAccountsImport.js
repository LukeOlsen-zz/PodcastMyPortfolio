...
const onSubmit = (values, { setSubmitting, setErrors, setFieldError }) => {
  if (!isNullOrUndefined(values.datafile) && (values.datafile.type === 'text/csv' || values.datafile.type === 'application/vnd.ms-excel') || values.datafile.type === 'text/plain') {
    ...
    axios.post('/api/clientaccounts/import', fd, {
      headers: { ...authHeader() }
    })
      .then(function (response) {
        ...
        if (values.importResults.length === 1) {
          ...
          values.updateNoticeMessage = 'Client accounts imported';
      ...
      .catch(function (error) {
        ...
        if (error.response.status === 500) {
          ...
          values.updateNoticeVisible = true;
          values.updateNoticeMessage = 'Problem importing client accounts';
          values.updateNoticeStyle = 'danger';
          setSubmitting(false);
        }
      });
...
class ClientAccountsImport extends Component {
  constructor(props) {
    super(props);
    this.state = {
      datafile: null,
            ...
            ({
              ...
              setFieldValue
                ...
                <Form onSubmit={handleSubmit} noValidate name='simpleForm'>
                  <Card>
                    <CardHeader><i className="fa fa-user" /><strong>Client Accounts Import</strong>
                    </CardHeader>
                    <CardBody>
                      <FormGroup row>
                        <Col md="2">
                        ...
                        <Col xs="12" md="10">
                          <Input type="file"
                              ...
                              name="datafile"
                              id="datafile"
                              placeholder="Client Accounts File"
                          ...
                          <FormText>The import file must be in a TAB delimited format with these four fields [Your client id] [Your client's account id] [Descriptive account name] [General account name]</FormText>
                        </Col>
                      </FormGroup>
                    </CardBody>
                    <CardFooter>
...
export default ClientAccountsImport;
