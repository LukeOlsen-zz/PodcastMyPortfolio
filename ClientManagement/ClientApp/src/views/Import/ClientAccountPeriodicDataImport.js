...
const onSubmit = (values, { setSubmitting, setErrors, setFieldError }) => {
  if (!isNullOrUndefined(values.datafile) && (values.datafile.type === 'text/csv' || values.datafile.type === 'application/vnd.ms-excel') || values.datafile.type === 'text/plain') {
    ...
    axios.post('/api/clientaccountperiodicdata/import', fd, {
      headers: { ...authHeader() }
    })
      .then(function (response) {
        ...
        if (values.importResults.length === 1) {
          ...
          values.updateNoticeMessage = 'Client account periodic data imported';
      ...
      .catch(function (error) {
        ...
        if (error.response.status === 500) {
          ...
          values.updateNoticeVisible = true;
          values.updateNoticeMessage = 'Problem importing client account periodic data';
          values.updateNoticeStyle = 'danger';
          setSubmitting(false);
        }
      });
...
class ClientAccountPeriodicDataImport extends Component {
  constructor(props) {
    super(props);
    this.state = {
      datafile: null,
            ...
            ({
              ...
              setFieldValue
                ...
                <Form onSubmit={handleSubmit} noValidate name='simpleForm'>
                  <Card>
                    <CardHeader><i className="fa fa-user" /><strong>Client Account Periodic Data Import</strong>
                    </CardHeader>
                    <CardBody>
                      <FormGroup row>
                        <Col md="2">
                        ...
                        <Col xs="12" md="10">
                          <Input type="file"
                              ...
                              name="datafile"
                              id="datafile"
                              placeholder="Client Account Periodic Data File"
                          ...
                          <FormText>The import file must be in a TAB delimited format with these 3 fields [Your client's account id] [Data date] [Ending balance]</FormText>
                        </Col>
                      </FormGroup>
                    </CardBody>
                    <CardFooter>
...
export default ClientAccountPeriodicDataImport;
