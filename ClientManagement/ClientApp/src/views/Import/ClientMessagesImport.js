...
const onSubmit = (values, { setSubmitting, setErrors, setFieldError }) => {
  if (!isNullOrUndefined(values.datafile) && (values.datafile.type === 'text/csv' || values.datafile.type === 'application/vnd.ms-excel') || values.datafile.type === 'text/plain') {
    ...
    axios.post('/api/clientmessages/import', fd, {
      headers: { ...authHeader() }
    })
      .then(function (response) {
        ...
        if (values.importResults.length === 1) {
          ...
          values.updateNoticeMessage = 'Client messages imported';
      ...
      .catch(function (error) {
        ...
        if (error.response.status === 500) {
          ...
          values.updateNoticeVisible = true;
          values.updateNoticeMessage = 'Problem processing client messages import';
          values.updateNoticeStyle = 'danger';
          setSubmitting(false);
        }
      });
...
class ClientMessagesImport extends Component {
  constructor(props) {
    super(props);
    this.state = {
      datafile: null,
            ...
            ({
              ...
              setFieldValue
                ...
                <Form onSubmit={handleSubmit} noValidate name='simpleForm'>
                  <Card>
                    <CardHeader><i className="fa fa-user" /><strong>Client Message Import</strong>
                    </CardHeader>
                    <CardBody>
                      <FormGroup row>
                        <Col md="2">
                        ...
                        <Col xs="12" md="10">
                          <Input type="file"
                              ...
                              name="datafile"
                              id="datafile"
                              placeholder="Client Message File"
                          ...
                          <FormText>The import file must be in a TAB delimited format with these three or four fields [Your client id] [Message type (use NEWS or SUMMARY)] [Message] [Expiration date (optional)]</FormText>
                        </Col>
                      </FormGroup>
                    </CardBody>
                    <CardFooter>
...
export default ClientMessagesImport;
