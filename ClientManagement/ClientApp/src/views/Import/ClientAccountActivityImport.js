...
const onSubmit = (values, { setSubmitting, setErrors, setFieldError }) => {
  if (!isNullOrUndefined(values.datafile) && (values.datafile.type === 'text/csv' || values.datafile.type === 'application/vnd.ms-excel') || values.datafile.type === 'text/plain') {
    ...
    axios.post('/api/clientaccountactivities/import', fd, {
      headers: { ...authHeader() }
    })
      .then(function (response) {
        ...
        if (values.importResults.length === 1) {
          ...
          values.updateNoticeMessage = 'Client account activity imported';
      ...
      .catch(function (error) {
        ...
        if (error.response.status === 500) {
          ...
          values.updateNoticeVisible = true;
          values.updateNoticeMessage = 'Problem importing client account activity';
          values.updateNoticeStyle = 'danger';
          setSubmitting(false);
        }
      });
...
class ClientAccountActivityImport extends Component {
  constructor(props) {
    super(props);
    this.state = {
      datafile: null,
            ...
            ({
              ...
              setFieldValue
                ...
                <Form onSubmit={handleSubmit} noValidate name='simpleForm'>
                  <Card>
                    <CardHeader><i className="fa fa-user" /><strong>Client Account Activity Import</strong>
                    </CardHeader>
                    <CardBody>
                      <FormGroup row>
                        <Col md="2">
                        ...
                        <Col xs="12" md="10">
                          <Input type="file"
                              ...
                              name="datafile"
                              id="datafile"
                              placeholder="Client Account Activity File"
                          ...
                          <FormText>The import file must be in a TAB delimited format with these 4 or 5 fields [Your client's account id] [Activity date] [Activity type (TRANSFER, DEPOSIT, LOANPAY, or WITHDRAWAL)] [Activity amount] [Activity description override (optional)]</FormText>
                        </Col>
                      </FormGroup>
                    </CardBody>
                    <CardFooter>
...
export default ClientAccountActivityImport;
